...
header('Access-Control-Allow-Headers: Content-Type, Content-Range, Content-Disposition, Content-Description, Accept, AuthorizationToken');
    ...
    if ($pozycjaNaStronie != '1'){
        $q = "UPDATE `cms_page_element` SET `page_element_order` = `page_element_order` + 1 WHERE page_element_order = ('$pozycjaNaStronie'-1) AND page_container_id='$idKontenera'";
        $r = mysqli_query($abc, $q);
